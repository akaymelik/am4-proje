...
- **Cargo training default 0, `L_CAP_FACTOR = 0.7` (muhafazakar tahmin).** am4-cc'nin `× 1.06` max-training buffer'ı uygulanmaz. Gerekçe: gerçek dünyada koltuk/kargo %100 dolmadığı için %5 muhafazakar tahmin gerçeğe daha yakın; kullanıcı için "site fazla söz vermedi, gerçek daha iyi çıktı" sürpriz dengesi. Cargo training UI input olarak da açılmıyor (input yorgunluğu, `repair_training` default 0 paralel kararı). Kullanıcı CKC-VVZ A400M doğrulamasında am4-cc $217K'dan %5 düşük revenue ($207K) muhafazakar gerçekçilik olarak kabul edildi.
- **Reputation modellenmiyor (R=100% örtük varsayım).** Profit hesabında reputation çarpanı uygulanmaz. Gerekçe: AM4 oyun mekaniğinde R başlangıç %45, ilk 10 hub alımı + marketing ile %99'a yükselir; aktif oyuncu ortalamada %99 uçar (kullanıcı oyun gözlemi). Site optimal/max potansiyel gösterir, kullanıcı kendi profiline göre yorumlar (`repair_training` / `cargo_training` paralel felsefe). am4-cc da modellemiyor (Tier 2 paralel, bundle'da 0 hit). Memory #9 ve #10 yolcu+kargo doğrulamaları R=100 varsayımıyla yapılmıştı, hâlâ geçerli. UI input olarak da açılmıyor (input yorgunluğu, optimal varsayım yeterli).
- **Fuel ceil kanonik formül: `ceil(distance × 100) / 100`.** 0.01 km hassasiyetinde yukarı yuvarlama. Kaynak: abc8747 `route.cpp:463` + `formulae.md:479` (100% confidence, R²=1) + am4-cc bundle birebir (`Math.ceil(d * 100) / 100` helper). Tam-sayı km girdilerinde (`distances.bin` Uint16Array) etkisiz no-op; fraksiyonel distance'larda 0.01 km'ye yuvarlar. Eski `ceil(d/2)*2` semantic'i kanonik kaynakta dayanaksızdı (tek-km distance'lerde fuel'i ~%0.05 abartıyordu); Fix #6'da Tier 1+2 kanıtla düzeltildi. CO₂ formülü de aynı `ceil(d, 2)` semantic'ini kullanır (formulae.md:498/513) — Fix #7'de paralel uygulanacak.
- **CO₂ kalemi profit zincirinde (Fix #7, kanonik cpp formülü `route.cpp:472-490`).** `planes.js`'te her uçak için `co2` field'ı (`abc8747/am4 aircrafts.csv` kolon 11, range 0.05–0.35; 329/329 auto+manuel match Fix #3 paraleli). Pax: `co2_kg = [ceil(d,2) × ac.co2 × (y+2j+3f) + (y+j+f)] × (CI/2000+0.9)`. Cargo: `co2_kg = [ceil(d,2) × ac.co2 × (L/1000 + H/500) + (L+H)] × (CI/2000+0.9)` (L/H bizim sistemde lbs; cpp `cfg.l/100 × 0.7 × capacity` ≡ allocate_l). `co2_training = 0` hard-code (Reputation paralel: kanıtsız mekanizma modellenmiyor); `ac_load = 1.0` örtük (R=100 varsayım, optimal/max gösterim). `co2_cost = co2_kg / 1000 × co2_price`. **Default `co2_price = 150 $/1000kg`** (am4-cc Tier 2 default; PDF "min $100" iddiası Tier 3 reddedildi — kanonik onay yok). UI input anasayfa "Ekonomi Ayarları" bölümünde Yakıt ve CI arasında 3 sütunlu grid; `localStorage` `menoa_co2_price` persistence Fuel Price ile birebir aynı pattern. `worker.js` AI prompt CO₂ açıklaması bilinçli olarak güncellenmedi (kullanıcı kararı, sonraya).
- **Airport dual-code gösterimi (am4-cc paraleli).** Tüm UI'da `Şehir (IATA / ICAO), Ülke` formatı kullanılır (örn `Lima (LIM / SPIM), Perú`); `iata === icao` istisnasında tek kod gösterilir (sadece Lydd `EGMD` ve Charlotte Amalie `TIST` — gerçekten IATA'sı olmayan ICAO-only airport'lar). `Utils.formatAirportCode(ap)` ve `Utils.formatAirportLabel(ap)` helper'ları her display noktasında kullanılır. Search hem IATA hem ICAO kabul eder: `dataLoader.icaoToId` map'i ve `resolveIata` ICAO fallback'i (örn `SPIM` → `LIM`, `KILN` → `ILN`). `resolveHub` regex 3-4 harf kabul eder (`/^[A-Z]{3,4}$/`), datalist match `/\(([A-Z]{3,4})(?:\s*\/\s*([A-Z]{3,4}))?\)/`. **Internal lookup'lar (parquet id, distance, demand, am4-cc revenue zinciri) iata key üzerinden devam — değişmez.** Native `<datalist>` autocomplete tek satır içerikli; iki-satırlı zengin dropdown (am4-cc paraleli) custom widget gerektirir, ileride.
