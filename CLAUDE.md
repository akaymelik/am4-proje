...
- **CO₂ kalemi profit zincirinde (Fix #7, kanonik cpp formülü `route.cpp:472-490`).** `planes.js`'te her uçak için `co2` field'ı (`abc8747/am4 aircrafts.csv` kolon 11, range 0.05–0.35; 329/329 auto+manuel match Fix #3 paraleli). Pax: `co2_lbs = [ceil(d,2) × ac.co2 × (y+2j+3f) + (y+j+f)] × (CI/2000+0.9)`. Cargo: `co2_lbs = [ceil(d,2) × ac.co2 × (L/1000 + H/500) + (L+H)] × (CI/2000+0.9)` (L/H bizim sistemde lbs; cpp `cfg.l/100 × 0.7 × capacity` ≡ allocate_l). `co2_training = 0` hard-code (Reputation paralel: kanıtsız mekanizma modellenmiyor); `ac_load = 1.0` örtük (R=100 varsayım, optimal/max gösterim). `co2_cost = co2_lbs / 1000 × co2_price`. **Default `co2_price = 150 $/1000lbs`** (oyun B737-800 F=40 LTFD-ZUNZ testi V2 audit doğrulaması: oyun 88,434 vs sistem 88,412.8, fark %0.024 — birim lbs onaylandı). UI input anasayfa "Ekonomi Ayarları" bölümünde Yakıt ve CI arasında 3 sütunlu grid; `localStorage` `menoa_co2_price` persistence Fuel Price ile birebir aynı pattern. `worker.js` AI prompt CO₂ açıklaması güncellendi (lbs naming).
