...
- **Cargo training default 0 (kanonik), `L_CAP_FACTOR = 0.7`.** am4-cc'nin `× 1.06` max-training buffer'ı (effective factor 0.742) UYGULANMIYOR. Gerekçe: gerçek dünyada koltuk/kargo asla %100 dolmaz; %5 muhafazakar tahmin oyun-içi tipik koşula daha yakın (am4-cc CKC-VVZ A400M $217K vs bizim $207K — kullanıcı kararı: muhafazakar tahmin gerçekçi). Cargo training UI input olarak da açılmıyor — `repair_training` ile paralel karar (input yorgunluğu, ileride üyelik/profil ile).
